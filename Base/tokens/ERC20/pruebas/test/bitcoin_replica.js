...
describe("Bitcoin Replica Contract", function () {
	...
	describe("Transactions", function() {
		...
		it("Should mine until the reward is equal to 0", async function() {
			const { Bitcoin_Replica, owner } = await loadFixture(deployTokenFixture);
			...
			while (await Bitcoin_Replica.current_reward() != 0) {
				...
				console.log(await Bitcoin_Replica.current_reward());
				console.log(await Bitcoin_Replica.next_block());
