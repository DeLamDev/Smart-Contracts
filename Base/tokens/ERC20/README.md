...
1. Réplica de Bitcoin y sus mecanismos de emisión: Consiste en un token que replica el mecanismo de emisión de Bitcoin, consistiendo este en un suministro limitado a 21,000,000, comenzando con una recompensa al minar un bloque de 50 Bitcoins (siendo minado un bloque cada 10 minutos) y reduciendo a la mitad la recompensa cada 210,000 bloques (lo que se conoce como halving). Este contrato en realidad no replica  a la prefección a Bitcoin puesto que al estar limitados a 18 decimales no es posible representar correctamente todas las recompensas, por lo que nunca llega realmente a completar el suministro de 21,000,000, sino que se acerca con un 99.999% de precisión. La solución sería simple incrementando los decimales del contrato, pero hay una gran cantidad de aplicaciones que dan por sentado que se utiliza el estandar de 18 decimales, lo que potencialmente podría causar conflicto al interactuar con otros contratos y aplicaicones, por lo que decidí que era mejor perder un poco de precisión para conservar la compatibilidad. La forma en la que opera el contrato es que cada diez minutos permite minar un bloque pudiendo el minero determinar la dirección que recibirá la recompensa, siendo imposible minar previo al transcurso de 10 minutos posteriores al último bloque. En las pruebas puedes encontrar las simulaciones de su funcionalidad, lo que se hace en ellas es dar saltos artificiales en el tiempo para comprobar que están operando las condiciones fijadas. Notarás que hay una función denominada `mining_test()` que no está contemplada en el contrato original, la razón de esto es que al correr las simulaciones consumían demasiado tiempo y recursos computacionales (particularmente RAM), por lo que es probable que hubiera sido imposible utilizar las simulaciones para muchos usuarios, es por esto que decidí crear una nueva función que simplificara el procedimiento. 
